...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    if (len(db_user)==0):
        db_user = models.User(email=user["email"], name=user["name"], passhash=hashed_password)
        db.add(db_user)
    else: 
        db_user = db_user[0]
...
def check_user_password(db: Session, user_id: int, hashed_password: str):
    ...
    db_user = db.query(models.User).filter(models.User.id == user_id, models.User.passhash == hashed_password).first()
    return db_user
